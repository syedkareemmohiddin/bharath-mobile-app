...
const Home = ({ jobs, vendors, jobParts, sales, todayCollected, todayAdvances, todaySales, todayExpenses, todayPurchases, todayCashPurchases, todayPartsCost, todayVendorPayments, todayBankDeposits, todayBankWithdrawals, todayNetProfit, totalCollected, vendorPayable, today, setScreen, fetchAll, onMarkDelivered, onCollectBalance, onMarkReturned, onEditJob, onDeleteJob, onCollectAdvance, filteredTx, filterDateFrom, filterDateTo, setFilterDateFrom, setFilterDateTo, openingCash, saveOpeningCash, cashInHand, closingCash, dashDate, setDashDate, getDayData }) => {
  ...
  const d = isToday ? {
    ...
    bankDeposits: todayBankDeposits, bankWithdrawals: todayBankWithdrawals,
    expenses: todayExpenses, netProfit: todayNetProfit, opening: openingCash
  } : (dayData || { collected:0, advances:0, sales:0, cashPurchases:0, purchases:0, partsCost:0, vendorPayments:0, bankDeposits:0, bankWithdrawals:0, expenses:0, netProfit:0, opening:0 });
        ...
        {!(dashDate !== today && dayDataLoading) && (
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 16, marginBottom: 16 }}>
            ...
            {[
              ...
              ...((d.bankDeposits || 0) > 0 ? [{ label: 'Bank Deposit (Cash → Bank)', value: d.bankDeposits, color: '#f87171' }] : []),
              ...((d.bankWithdrawals || 0) > 0 ? [{ label: 'Bank Withdraw (Bank → Cash)', value: d.bankWithdrawals, color: '#4ade80' }] : []),
            ].map((row, i) => (
